/**
 * serum 2記事の掲載結果を記事ページと同じロジックでダンプ（評価用）。
 * apps/web: pnpm exec tsx scripts/evaluate-serum-articles.ts
 */
...
function pickItems(
  ...
  cat: (typeof CATEGORY_CONFIG)["serum"]
...
async function main() {
  ...
  const general = getArticleSpecBySlug("korean-serum-ranking-compare");
  const bright = getArticleSpecBySlug("korean-serum-brightening-ranking-compare");
  const cat = CATEGORY_CONFIG.serum;
  if (!general || !bright || !cat) throw new Error("spec missing");
  ...
  const itemsBright = pickItems(data.items, bright, cat);

  const gSet = new Set(itemsGeneral.map((i) => i.goodsNo));
  const bSet = new Set(itemsBright.map((i) => i.goodsNo));
  ...
  function dump(label: string, rows: RankingItemWithProduct[]) {
    ...
    for (const r of rows) {
      const name = (r.nameJa || r.name || "").replace(/\s+/g, " ").slice(0, 80);
      console.log(
        `rank=${r.rank}\t${r.goodsNo}\t${name}`
      );
    }
  }

  dump("korean-serum-ranking-compare (総合)", itemsGeneral);
  dump("korean-serum-brightening-ranking-compare (テーマ)", itemsBright);

  console.log("\n--- 共通 goodsNo ---");
  console.log(JSON.stringify(intersection));
  console.log("共通件数:", intersection.length, "/ 総合", itemsGeneral.length, "/ bright", itemsBright.length);

  const pool = data.items
    .filter((item) => scoreProductForCategory(toMinimal(item), cat) >= THRESHOLD)
    .sort((a, b) => a.rank - b.rank);
  console.log("\n--- serum 候補プール（スコア>=2）件数:", pool.length);
}
